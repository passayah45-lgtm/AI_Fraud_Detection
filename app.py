...
- **Fraud rate**: 0.17% (highly imbalanced, handled with SMOTE)
    ...
    if total < 10 or frauds == 0:
        st.info("Not enough predictions yet. Make predictions first.")
    else:
        try:
...
# PAGE: FRAUD DETECTION
    ...
    rc1.metric("Risk Profile",       risk_choice)
    ...
    with tab1:
        ...
        with col2:
            st.markdown(f"**Active threshold:** `{threshold}` ({risk_choice})")
    ...
    with tab2:
        ...
        if uploaded:
            ...
            df_up['Risk_Level']          = [risk_label_fn(p) for p in probas]
            ...
            if fraud_count == 0:
                ...
                st.warning(f"{fraud_count} suspicious transactions flagged at {risk_choice} threshold.")
